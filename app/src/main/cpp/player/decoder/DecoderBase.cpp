...
void DecoderBase::Start() {
    if (m_Thread == nullptr) {
        ...
        //互斥锁，由于RAII机制，在lock对象生命周期结束时候会将锁释放
        std::unique_lock<std::mutex> lock(m_Mutex);
        //修改解码状态为正在解码
        m_DecoderState = STATE_DECODING;
        //信号量，通知挂起的解码线程停止等待，开始解码
        m_Cond.notify_all();
    }
}
...
void DecoderBase::UnInit() {
    ...
    if (m_Thread) {
        Stop();
        //谁调用了这个函数？调用了这个函数的线程对象，
        // 一定要等这个线程对象的方法（在构造时传入的方法）执行完毕后（或者理解为这个线程的活干完了！），这个join()函数才能得到返回。
        //在什么线程环境下调用了这个函数？
        // 上面说了必须要等线程方法执行完毕后才能返回，那必然是阻塞调用线程的，
        // 也就是说如果一个线程对象在一个线程环境调用了这个函数，那么这个线程环境就会被阻塞，
        // 直到这个线程对象在构造时传入的方法执行完毕后，才能继续往下走，另外如果线程对象在调用join()函数之前，
        // 就已经做完了自己的事情（在构造时传入的方法执行完毕），那么这个函数不会阻塞线程环境，线程环境正常执行。
        m_Thread->join();
        delete m_Thread;
        m_Thread = nullptr;
    }
...
void DecoderBase::DoAVDecoding(DecoderBase *decoder) {
    ...
    do {
        ...
        //对于音频：初始化重采样上下文；对于视频：初始化图像格式转换上下文
...
void DecoderBase::SetLooping(bool isLooping) {
